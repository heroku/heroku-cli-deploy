...
function deploy(context, args) {
  return new Promise((resolve, reject) => {
    ...
    spawned.stderr.on('data', (chunk) => {
      if (!chunk.toString().startsWith('SLF4J:')) {
        cli.console.writeLog(chunk.toString());
      }
    });
    spawned.stdout.on('end', () => {});
    spawned.stderr.on('end', () => {
      cli.error(`There was a problem deploying to ${cli.color.white.bold(context.app)}.`)
      cli.error(`Make sure you have permission to deploy by running: ${cli.color.magenta('heroku apps:info -a ' + context.app)}`);
    });
  });
}
